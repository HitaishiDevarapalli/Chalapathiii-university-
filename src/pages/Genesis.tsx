...
export default function Genesis() {
  ...
  const historyData = aboutContent?.history;
  const heroTagline = historyData?.heroTagline || "Our Genesis";
  const heroTitle = historyData?.heroTitle || "A Journey of Vision, \nValues & Transformation";
  const heroSubtitle = historyData?.heroSubtitle || "From a vision rooted in knowledge to a future-ready multidisciplinary university driven by innovation, research, and global excellence.";
  const timelineSteps = (historyData?.milestones && historyData.milestones.length > 0) ? historyData.milestones : DEFAULT_GENESIS_STEPS;
  const brighterTomorrow = historyData?.brighterTomorrow;

  const nepData = historyData?.nep2020;
  const nepHeading = nepData?.heading || "A University Inspired by NEP 2020";
  const nepSubheading = nepData?.subheading || "Experiencing education as a living, interconnected ecosystem of knowledge, skills, and innovation.";
  const nepCoreBadge = nepData?.coreBadge || "NEP 2020";
  const nepNodesList = nepData?.nodes && nepData.nodes.length > 0 ? nepData.nodes : [
  ...
  const academicData = historyData?.academicEvolution;
  const academicHeading = academicData?.heading || "Academic Evolution";
  const academicSubheading = academicData?.subheading || "Three schools designed for the future of industry and research.";
  const academicSchools = academicData?.schools && academicData.schools.length > 0 ? academicData.schools : [
  ...
  const innovationEcoData = historyData?.innovationEcosystem;
  const innovationEcoHeading = innovationEcoData?.heading || "Innovation Ecosystem";
  const innovationEcoSteps = innovationEcoData?.steps && innovationEcoData.steps.length > 0 ? innovationEcoData.steps : [
    "Student", "Idea", "Innovation Lab", "Prototype", "Research", "Incubation", "Startup", "Industry", "Global Impact"
  ];

  const infraData = historyData?.innovationInfrastructure;
  const infraHeading = infraData?.heading || "Innovation Infrastructure";
  const infraSubheading = infraData?.subheading || "State-of-the-art facilities designed to transition students from passive recipients into active innovators.";
  const infraItems = infraData?.items && infraData.items.length > 0 ? infraData.items : [
  ...
  const visionTomorrowData = historyData?.visionForTomorrow;
  const visionTomorrowHeading = visionTomorrowData?.heading || "Our Vision for Tomorrow";
  const visionTomorrowParagraph = visionTomorrowData?.paragraph || "Chalapathi University is envisioned as a multidisciplinary institution where innovation meets purpose, research fuels progress, and every learner is empowered to become a leader capable of creating meaningful impact across the world.";
    ...
    <div ref={containerRef} className="min-h-screen bg-[#F8FAFC] text-[#0F172A] font-[var(--font-poppins)] overflow-hidden">
      ...
      <section className="relative w-full min-h-[140vh] md:min-h-[160vh] bg-gradient-to-b from-[#ffffff] via-[#f0f9ff] to-[#e0f2fe] overflow-hidden pt-32 pb-40 px-5 flex flex-col justify-start">
        ...
        <div className="max-w-[1400px] mx-auto w-full relative z-20 mt-20 md:mt-16 h-auto md:h-[650px] flex flex-col md:flex-row items-center md:items-end justify-between gap-24 md:gap-6 lg:gap-8 pb-10 pt-10 md:pt-0">
          ...
          {timelineSteps.map((step, i) => {
            ...
            const calculatedHeight = step.height || `${30 + i * 15}%`;
            const imgSrc = step.img?.startsWith("/") || step.img?.startsWith("http") || step.img?.startsWith("data:") 
              ? step.img 
              : `/${step.img || "genesis/1995.jpg"}`;
            ...
            return (
              <motion.div 
                ...
                transition={{ duration: 1.2, delay: 0.2 + (i * 0.2), type: "spring", bounce: 0.2 }}
                ...
                style={{ '--md-height': calculatedHeight } as React.CSSProperties}
              >
                ...
                <div className="w-full h-full relative group perspective-1000">
                  {/* 3D Top Face */}
                  <div className={`absolute top-0 left-0 w-full h-10 md:h-16 -mt-5 md:-mt-8 rounded-t-2xl z-20 ${isLast ? 'bg-gradient-to-br from-[#1E4B9C] to-[#0A2044]' : 'bg-white/60 backdrop-blur-xl border-white'} shadow-[inset_0_2px_10px_rgba(255,255,255,0.9),0_-10px_30px_rgba(18,58,122,0.1)] border-t border-x ${isLast ? 'border-[#38bdf8]/50' : 'border-white/80'} skew-x-12 transform origin-bottom-left transition-transform duration-500 group-hover:brightness-110`}></div>
                  
                  {/* 3D Front Face */}
                  <div className={`absolute top-5 md:top-8 left-0 w-full h-[calc(100%-1.25rem)] md:h-[calc(100%-2rem)] rounded-b-2xl z-10 p-4 md:p-8 flex flex-col items-center justify-start ${isLast ? 'bg-gradient-to-b from-[#123A7A] to-[#081b3a] border-[#123A7A] shadow-[0_30px_60px_rgba(18,58,122,0.4)]' : 'bg-white/80 backdrop-blur-2xl border-white/80 shadow-[0_20px_50px_rgba(0,0,0,0.05)]'} border-x border-b overflow-hidden transition-colors duration-500`}>
                    
                    {/* Subtle inner reflection */}
                    <div className="absolute inset-0 bg-gradient-to-b from-white/20 to-transparent pointer-events-none rounded-b-2xl"></div>

                    <div className={`text-center py-1 md:py-2 px-3 md:px-5 rounded-full inline-flex items-center justify-center shadow-inner backdrop-blur-md mb-3 md:mb-4 ${isLast ? 'bg-white/10 text-white border border-white/20' : 'bg-[#123A7A]/5 text-[#123A7A] border border-[#123A7A]/10'}`}>
                      <h3 className="font-[800] tracking-wider text-[11px] md:text-[14px]">{step.year}</h3>
                    </div>
                    
                    <h4 className={`font-[900] text-[13px] md:text-[18px] text-center leading-tight mb-2 ${isLast ? 'text-white' : 'text-[#0F172A]'}`}>
                      {step.title}
                    </h4>

                    {/* Decorative line */}
                    <div className={`h-[2px] w-8 rounded-full mb-3 block ${isLast ? 'bg-sky-400/50' : 'bg-[#123A7A]/20'}`}></div>

                    <p className={`text-[12px] md:text-[13px] leading-relaxed text-center font-[500] block ${isLast ? 'text-white/80' : 'text-[#0F172A]/70'}`}>
                      {step.desc}
                    </p>
                  </div>
                </div>
        ...
        >
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-10 mb-16 border-b border-gray-100 pb-12 relative z-10">
            <div className="max-w-md">
              <p className="text-[#0F172A]/40 font-[700] uppercase tracking-[0.2em] text-[12px] md:text-[14px] mb-3">
                {brighterTomorrow?.tagline || "Towards a"}
              </p>
              <h2 className="text-[36px] md:text-[48px] font-[900] text-[#123A7A] leading-[1.1] tracking-tight">
                {brighterTomorrow?.heading || "Brighter Tomorrow"}
              </h2>
            </div>
            <p className="text-[16px] md:text-[20px] text-[#0F172A]/70 font-[400] max-w-2xl leading-relaxed border-l-2 border-sky-200 pl-8">
              {brighterTomorrow?.description || "Guided by its founding values and inspired by innovation, Chalapathi University continues to evolve into a multidisciplinary institution dedicated to academic excellence, research, entrepreneurship, and global impact."}
            </p>
          </div>
          ...
          >
            {(brighterTomorrow?.pillars && brighterTomorrow.pillars.length > 0 
              ? brighterTomorrow.pillars 
              : [
                { title: "Knowledge" },
                { title: "Innovation" },
              ...
              return (
                ...
                >
                  ...
                  {item.desc && (
                    <p className="text-[13px] text-gray-500 leading-relaxed relative z-10 font-[400]">
                      {item.desc}
                    </p>
                  )}
                </motion.div>
                ...
                <>
                  ...
                  >
                    ...
                    <div className="relative z-10 flex flex-col items-center">
                      <span className="font-[900] text-[24px] md:text-[28px] text-transparent bg-clip-text bg-gradient-to-b from-white to-sky-200 tracking-tight leading-none text-center px-2">
                        {nepCoreBadge}
                      </span>
                    </div>
                  </motion.div>
      ...
      <section className="py-32 px-5 bg-[#F8FAFC]">
        <div className="max-w-[1200px] mx-auto">
          ...
          >
            <h2 className="text-[32px] md:text-[48px] font-[800] text-[#123A7A]">{academicHeading}</h2>
            <p className="text-[#0F172A]/60 font-[500] mt-4">{academicSubheading}</p>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {academicSchools.map((school, i) => {
              const colors = [
      ...
      <section className="py-32 bg-white border-y border-[#E7EDF4] overflow-hidden">
        <div className="max-w-[1400px] mx-auto px-5 text-center">
          ...
          >
            {innovationEcoHeading}
          </motion.h2>

          <div className="flex flex-wrap justify-center items-center gap-4 md:gap-8">
            {innovationEcoSteps.map((step, i, arr) => (
      ...
      <section className="py-32 bg-[#F8FAFC]">
        <div className="max-w-[1200px] mx-auto px-5">
          <motion.div 
            ...
            className="mb-16"
          >
            <h2 className="text-[32px] md:text-[48px] font-[800] text-[#123A7A]">{infraHeading}</h2>
            <p className="text-[#0F172A]/60 font-[500] mt-4 max-w-2xl">{infraSubheading}</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-4 h-auto md:h-[600px]">
            ...
            >
              <div 
                className="absolute inset-0 bg-cover bg-center opacity-20 mix-blend-overlay group-hover:opacity-40 transition-opacity duration-500"
                style={{ backgroundImage: `url(${infraItems[0]?.img || 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&q=80'})` }}
              ></div>
              <div className="relative z-10 flex flex-col h-full justify-end">
                <Lightbulb size={40} className="mb-4 text-[#D32F2F]" />
                <h3 className="text-[28px] font-[800] mb-2">{infraItems[0]?.title || "Innovation Centre"}</h3>
                <p className="text-white/80 font-[500] text-[15px]">{infraItems[0]?.desc || "Seed-stage mentoring, prototyping support, and startup incubation."}</p>
              </div>
            </motion.div>
            ...
            >
              ...
              <h3 className="text-[18px] font-[800] text-[#123A7A] mb-1">{infraItems[1]?.title || "Centres of Excellence"}</h3>
              <p className="text-[#0F172A]/60 text-[13px] font-[500]">{infraItems[1]?.desc || "AI/ML, IoT & Robotics"}</p>
            </motion.div>
            ...
            >
              ...
              <h3 className="text-[18px] font-[800] mb-1">{infraItems[2]?.title || "Maker Space"}</h3>
              <p className="text-white/80 text-[13px] font-[500]">{infraItems[2]?.desc || "Hands-on fabrication"}</p>
            </motion.div>
            ...
            >
              ...
              <div>
                <h3 className="text-[20px] font-[800] text-[#123A7A] mb-1">{infraItems[3]?.title || "E-Cell & Industry Interface"}</h3>
                <p className="text-[#0F172A]/60 text-[14px] font-[500]">{infraItems[3]?.desc || "Structured venture-creation training and continuous industry alignment."}</p>
              </div>
            </motion.div>
          </div>
        </div>
